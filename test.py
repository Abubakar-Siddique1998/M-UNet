...
test_args = IsprsConfigs(net_name= 'm-unet', 
                         data='Vaihingen',  # Potsdam, Vaihingen
                         bands_list=['IRRG'],    # RGB, IRRG, 'DSM'
                         note='test-outputs'
                         )

test_args.snapshot = "saved_models/vaihingen116.pth"
test_args.input_size = [256, 256] 
test_args.val_batch = 8
...
def main():
    ...
    net=munet().cuda()
    label_values=["Buildings","Trees","Low veg.","Road surf","Cars"]
    ...
    scores = compute_scores(cm, label_values)
    ...
    for img, gt, gt_nb in tqdm(zip(test_images, test_labels, eroded_labels), total=len(test_set[GTNB]), leave=False):
        ...
        with torch.no_grad():
            for i, coords in enumerate(tqdm(grouper(batch_size, sliding_window(img, step=stride, window_size=window_size)), total=total, leave=False)):
                ...
                # Do the inference
                raw_predictions = net(image_patches)
                raw_predictions = nn.Softmax(dim=1)(raw_predictions)
                # outs = raw_predictions.argmax(dim=1)
                outs = raw_predictions.data.cpu().numpy()
                b, _, _, _ = outs.shape
...
def metrics(predictions, gts, label_values=LABELS):
    ...
    a_del = np.delete(cm, 3, 0)
    cm = np.delete(a_del, 3, 1)
    label_values=["Buildings","Trees","Low veg.","Road surf","Cars"]
    cm=cm[0:5,0:5]
...
def compute_scores(conmatrix=None, col_idx=None, savefile=None):
    if col_idx is None:
        col_idx = ["Buildings","Trees","Low veg.","Road surf","Cars"]
    ...
    scores['Avg1'] = scores.mean(axis=1)
